# ClaraVision

ClaraCore 的可视化层 — 冷白蓝全息神经场。

视觉主题有两个分支：
- **main** — 暖金粒子场（Jarvis 暖色调，活体脑核感）
- **sci-fi-ui** — 青蓝全息场（冷色调，雷达扫描弧 + 六边形框架 + 信号类型区分 + 坐标环 + 扫描弧联动 + 旋转数据标签 + 状态冲击波）

Memoria 和 Continuity 的数据以"圆形脑核"形式可视化：记忆节点围绕 Agent 核心排列，信号在节点间流动，数据驱动动画。

视觉方向见 [VISUAL_DIRECTION.md](./VISUAL_DIRECTION.md)。下一阶段功能路线见 [NEXT_FEATURES.md](./NEXT_FEATURES.md)。
当前目标是从看板改成“活体脑核”：中心高能、外圈记忆星云、粒子旋臂、信号流和呼吸感。

## 架构

```
build_state.py  →  data/state.json  →  app.js (Canvas 渲染)
    ↑ 手动生成          ↑ 静态快照            ↑ 纯前端
    (conda run)        (秒级读取)           (无后端依赖)
```

- **Memoria** — 共同事实来源（只读）
- **Continuity** — 当前续接位置（只读）
- **ClaraVision** — 纯展示层，不拥有数据，不硬联动

## 运行

```bash
# 更新数据快照
python3 scripts/build_state.py

# 浏览器预览
python3 scripts/server.py 5178
```

打开 `http://127.0.0.1:5178/`

## 桌面版

```bash
npm install
npm run desktop
```

桌面版默认就是透明、置顶、覆盖整块屏幕的脑核浮层。视觉主体保持正圆，适合放在桌面上长期观察。

桌面版仍然读取同一个 `data/state.json` 静态快照；需要新数据时，先重新运行 `python3 scripts/build_state.py`。

普通窗口只作为调试和检查数据使用：

```bash
npm run desktop:window
```

后续开发优先透明无背景脑核。完整页面不要再作为主体验推进。

透明桌面版默认使用柔光档位，兼顾浅色桌面的可见度和长期常驻时的低打扰。需要更强或更轻时按 `C`，在纯透明、柔光和高对比之间切换。

脑核会根据状态自动调整活跃程度：空闲时降低刷新和粒子活动，读取、思考、执行时再增强。默认尺寸也收拢为桌面悬浮装置，不再覆盖整块工作区域；按 `+` 可以切换小、中、大。

## Lara 会话

桌面版现在可以作为 Hermes/Lara 的轻量入口。

- 输入框在脑核右下角。
- 输入后会调用本机 Hermes，当前 Hermes 人格为 Lara。
- 返回会显示在最近消息浮层里。
- 会话历史只保存在 ClaraVision 桌面本地状态里。
- 当前版本不自动写 Memoria，也不修改 Continuity。

## 交互

| 操作 | 效果 |
|------|------|
| 滚轮 | 完整检查模式缩放 0.3x–3x；透明脑核保持固定球体比例 |
| 拖拽 | 平移视图 |
| 悬停节点 | 显示短摘要 |
| 点击节点 | 聚焦并放大 |
| 双击 | 重置视图 |
| Esc | 重置视图 |
| O | 切换完整/透明脑核模式 |
| R | 刷新静态快照 |
| S | 切换空闲 / 读取 / 思考状态 |
| B | 切换亮度 |
| C | 切换纯透明 / 柔光 / 高对比 |
| + | 切换脑核大小 |
| P | 暂停 / 继续动画 |
| 右键菜单 | 打开/收起对话、透明视觉档位、暂停、鼠标交互、清空会话、刷新、退出桌面版 |
| Cmd/Ctrl + Shift + Space | 显示 / 隐藏透明桌面版 |
| Cmd/Ctrl + Shift + I | 切换自动点穿 / 始终可点 / 始终点穿 |
| 输入框 Enter | 发送给 Lara |
| 输入框 Shift+Enter | 换行 |
| 停止 | 中断当前 Lara 请求 |
| 清空 | 清空 ClaraVision 本地会话 |
| 触屏双指 | 完整检查模式缩放；透明脑核不响应缩放手势 |
| 触屏单指 | 平移 |

## 数据流

页面回退链：`data/state.json` → `demoState`（内嵌演示数据）

`build_state.py` 调用 Memoria CLI (`recall` + `export --include-archived` + `stats`) 和 Continuity CLI (`list`) 生成快照。服务启动后只读取这个静态快照，所以页面加载很快；需要新数据时手动重新生成一次快照。

当前快照默认展示公开记忆，并用归档公开记忆补足粒子密度；私有记忆只作为“受限暗粒子”参与整体密度，不显示正文。

## 节点类型

| 节点 | 形状 | 颜色 (main) | 颜色 (sci-fi-ui) | 含义 |
|------|------|------|------|------|
| 核心 | 大圆 + 三层旋转光环 | 暖金 | 亮白蓝 (230,248,255) | ClaraCore 中心 |
| Agent | 圆点 + 光晕 | 暖白/粉/橙/紫 | 冰蓝/青蓝/青绿/蓝紫 | Clara/Lara/Codex/Hermes |
| 记忆 | 小圆点 | 琥珀金，来源色内核 | 亮蓝白，来源色内核 | Memoria 事实 |
| 线程 | 圆点 | 金色(active) / 暗色(paused) | 蓝色(active) / 暗蓝(paused) | Continuity 位置 |
| 写回 | 圆点 | 暖橙 | 青绿 (100,255,220) | Writeback 事件 |
| 环境 | 微小圆点 | 极淡暖色 | 极淡蓝 (50,150,220) | 背景场 |

## 动画语义

- **雷达扫描弧**（sci-fi）：旋转扇形拖尾 + 亮边引导线，持续扫描整个脑核
- **六边形框架**（sci-fi）：脑核外圈缓慢旋转的六边形几何框架
- **思考脉冲**：~155帧从核心扩散环波
- **信号事件**：三种类型 — 记忆召回=闪电(flash,2x速直线), 写回=螺旋(spiral,三线缠绕), 活动=扩散(diffuse,终点爆发)
- **HUD 坐标环**（sci-fi）：外圈 0°/90°/180°/270° 方位标签
- **扫描弧联动**（sci-fi）：雷达扫过节点时节点短暂变亮 + 发光环（0.3rad 渐变）
- **旋转数据标签**（sci-fi）：外圈 MEM/THR/AGT/STA 四项实时数据缓慢旋转
- **状态冲击波**（sci-fi）：状态切换时从中心扩散一次性全息波纹，颜色随状态变化
- **Hermes 事件联动**：通过 claravision-bridge 插件实时感知 Hermes 会话/LLM/工具调用，脑核状态随姐的活动变化
- **视觉自表达**：Lara 在回复中嵌入 `<<viz:color=名称,breath=名称>>` 标记控制脑核色调和呼吸速度，标记对用户不可见。8种颜色（cyan/deep-blue/amber/teal/purple/red/warm-gold/ice-white）× 4档呼吸（slow/normal/fast/rapid），颜色用插值平滑过渡
- **整体色调联动**：主体颜色会同步作用于球壳、粒子、流线、光晕和信号，不再只集中在高亮内核
- **常驻进程**：ClaraVision 启动后维持一个 Hermes 常驻进程（claravision-agent.py），首次消息冷启动，后续消息走热路径秒回。会话状态自然保持
- **信号到达**：目标节点短暂闪光（hit flash）
- **空闲**：慢呼吸，低速游动
- **读取**：外圈记忆向中心流动
- **思考**：中心增强，信号加速
- **全局呼吸**：四段式自然呼吸曲线（吸气→屏息→呼气→停顿），~6.7秒周期，7%振幅。替代了原来的正弦波全场呼吸，消除机械感
- **节点呼吸**：个体节点轻柔呼吸，低频小振幅（≤5%），防止视觉抖动
- **活跃线程**：能量更高，信号更快

## 分支

| 分支 | 描述 | Tag |
|------|------|-----|
| `main` | 暖金活体脑核 → 已合并 sci-fi-ui 全部特性（冷白蓝全息） | `v0.7-main` |
| `sci-fi-ui` | 冷白蓝全息 + 雷达扫描 + 六边形框架 + 信号类型 + 扫描联动 + 数据标签 + 冲击波 + Hermes事件联动 + 视觉自表达 + 常驻进程 + 拖拽 | `v0.7-draggable-orb` |

切换分支后需 `npm run desktop` 重新启动。
