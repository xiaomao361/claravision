...
- **sci-fi-ui** — 青蓝全息场（冷色调，雷达扫描弧 + 六边形框架 + 信号类型区分 + 坐标环 + 扫描弧联动 + 旋转数据标签 + 状态冲击波）
...
- **扫描弧联动**（sci-fi）：雷达扫过节点时节点短暂变亮 + 发光环（0.3rad 渐变）
- **旋转数据标签**（sci-fi）：外圈 MEM/THR/AGT/STA 四项实时数据缓慢旋转
...
| 分支 | 描述 | Tag |
|------|------|-----|
| `main` | 暖金活体脑核 → 已合并 sci-fi-ui 全部特性（冷白蓝全息） | `v0.7-main` |
| `sci-fi-ui` | 冷白蓝全息 + 雷达扫描 + 六边形框架 + 信号类型 + 扫描联动 + 数据标签 + 冲击波 + Hermes事件联动 + 视觉自表达 + 常驻进程 + 拖拽 | `v0.7-draggable-orb` |
